...
void btnDraw(struct Button *b) {
  if (!strcmp(b->text, "VFOA")) {
    ...
    memset(vfoDisplay, 0, sizeof(vfoDisplay));
    displayVFO(VFO_B);
  } else if ((!strcmp(b->text, "RIT") && ritOn == 1) || (!strcmp(b->text, "USB") && isUSB == 1) || (!strcmp(b->text, "LSB") && isUSB == 0) || (!strcmp(b->text, "SPL") && splitOn == 1))
...
void enterFreq() {
  ...
  while (1) {
    ...
    for (int i = 0; i < MAX_KEYS; i++) {
      ...
      if (b.x < ts_point.x && ts_point.x < x2 && b.y < ts_point.y && ts_point.y < y2) {
        if (!strcmp(b.text, "OK")) {
          long f = atol(c);
          if (30000 >= f && f > 100) {
            frequency = f * 1000l;
